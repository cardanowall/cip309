...
vectors are pinned under [`../conformance/`](../conformance). Where this prose
...
The error codes named above are defined in [`../registries/error-codes.json`](../registries/error-codes.json) and in [Structural validation, verifier roles, and error codes](#structural-validation-verifier-roles-and-error-codes).
...
content AEAD is [`../registries/aead-algorithms.json`](../registries/aead-algorithms.json).
...
internal building block in [`../registries/kdf-algorithms.json`](../registries/kdf-algorithms.json)
...
[`../registries/error-codes.json`](../registries/error-codes.json).
...
[`../conformance/sealed-poe/`](../conformance/sealed-poe).
...
2. **Schema parse.** The decoded value is run through a closed-schema parser ([`../schemas/`](../schemas)). The schema is the structural authority: type checks, length bounds (the 64-byte chunk limit on chunked byte/text arrays, the 32-byte length checks on `supersedes` and on commitment roots), and a strict object mode that rejects unknown fields. The schema imposes **no** numeric cap on `items[]`, `sigs[]`, or `slots[]` entry counts — the only ceiling is the ledger's maximum transaction size, enforced at submission by Cardano nodes; a validator MUST NOT emit an error solely because an entry count is high.
