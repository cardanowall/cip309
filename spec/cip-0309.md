...
Status:         Draft
Authors:        ingvarson
Implementors:   (to be listed at submission)
Discussions:    (to be added)
Created:        (placeholder — to be set at submission)
License:        CC-BY-4.0
```
...
1. **Content-first.** The cryptographic hash of the content is the primary assertion; every other field is metadata *about* that hash.
...
[`../cddl/cip-309.cddl`](../cddl/cip-309.cddl); per-component JSON Schemas in
...
CBOR map invariant in [`../cddl/cip-309.cddl`](../cddl/cip-309.cddl)). A
...
Signers and verifiers **MUST** agree on this canonicalisation bit-for-bit; implementations **SHOULD** use a CBOR library that supports RFC 8949 deterministic encoding natively. The complete grammar for the canonical record body is the CDDL at [`../cddl/cip-309.cddl`](../cddl/cip-309.cddl); conformance vectors that pin exact canonical bytes are at [`../conformance/`](../conformance).
...
block time of its transaction alone; a signature adds *who attests to this
record body*, and its absence does not weaken the existence claim. CIP-309 v1
...
to; no signer attests to *which* other signers co-signed. Applications needing a
...
> **exactly 32 bytes**, it *is* the raw Ed25519 public key, not a pointer to
...
Ed25519 *secret seed* is 32 bytes, while the *expanded private key* is
...
machine artifact [`../cddl/cip-309.cddl`](../cddl/cip-309.cddl); the cross-field
...
attestation would only prove that *some* ciphertext existed.
...
holder of a matching `priv_R` can confirm *what the commitment is to*; the rest of
...
- `salt` length 16–64 bytes inclusive (`ENC_PASSPHRASE_SALT_TOO_SHORT` /
  ...
  cap).
- `params = { m, t, p }` with `m ≥ 65536` (KiB ≈ 64 MiB), `t ≥ 3`, `p ≥ 1`; any
...
that opts into the multi-hash pattern (`sha2-256` *and* `blake2b-256` in the
...
*some* leaf set with that root existed at the block time; it does not let any
...
[Record-level signatures (COSE_Sign1)](#record-level-signatures-cose_sign1)).
Replay against a *different* record is detectable: the signature does not verify
against the substituted signing input. There is no anti-replay across
*identical* records — the same signed body MAY be submitted in two transactions.
...
the ciphertext bytes *as published* comes from the storage layer — an IPFS CIDv1
...
- **Hidden recipient keys.** Recipient public keys are not on the wire; an
  observer learns only `slots.length`, never *who* the record was sealed to.
...
identity X25519 key — the two keys derive from the same seed via *different*
...
and this durability is load-bearing. The consequence is forward-looking: *new*
...
record's `items` content claims normally. Implementations that *do* support
...
- **No issuer field.** The PoE claim is about content, not about who submitted
  ...
  record-level signatures: the signer's public key *is* the cryptographic
...
level — multiple signers MAY independently endorse the *same* whole record body,
