// Label 309 v1 reference implementation — structural record validator
// Spec: Label 309 (record schema + structural validation).
...
// is registered in Label 309 §4.10.7; a deployment wishing to accept `-19`
...
// (Label 309 §4.5) and are governed by MERKLE_COMMIT_ALGS below.
...
// Registered Merkle list-commitment algorithms (per Label 309 §4.10.3). The
...
// surfaces as `UNSUPPORTED_MERKLE_COMMIT_ALG` per Label 309.
...
// Forward-compat extension-key tolerance (Label 309 §4.1.4). Keys matching either
...
// Top-level base keys defined in Label 309 §4.1. Any unknown key that is NOT an
...
// Per Label 309 §4.8 and the CDDL `bstr .size (1..64)` / `tstr .size (1..64)` shapes,
...
// Each entry of `record.sigs` is a closed CBOR map { cose_sign1, cose_key? } per Label 309 §4.6.3.
...
// Per Label 309 §4.2: `hashes` is a non-empty CBOR map of <hash-alg-id> → <digest>.
...
// HashesMapSchema rationale). Final identifier per Label 309 §4.10.6.
const KNOWN_PASSPHRASE_KDF_ALGS = new Set(['argon2id']);

// ENC_PASSPHRASE_PARAMS_EXCEED_POLICY (Label 309, citing Label 309 §4.10.6) is
...
const EncryptionEnvelopeSchema = z
  .object({
    ...
    aead: z.string(),
    // Envelope-level `kem` governs every entry in `slots[]` (per Label 309 §4.4).
    kem: z.string().optional(),
    nonce: z.instanceof(Uint8Array),
    // Wire field name: `slots` (per Label 309 §4.4 — recipient pubkeys are NOT
  ...
  .strict()
  .superRefine((enc, ctx) => {
    // `enc.scheme` MUST be the unsigned integer 1 (Label 309 §4.4). Reject CBOR
    ...
    if (/aes-cbc/i.test(enc.aead)) {
      ctx.addIssue({
        code: 'custom',
        path: ['aead'],
        message: 'AES-CBC is unauthenticated; Label 309 mandates an authenticated cipher',
...
// Content-hash identifiers (per Label 309 §4.10.2). An `enc`-bearing item's
// `hashes` map MUST contain at least one entry from this set — see
// Label 309 §4.4. Every entry of HASH_ALGS is a content-hash; list commitments
...
const ItemEntrySchema = z
  .object({
    ...
    // Per Label 309 §4.8, every URI is a chunked-tstr-array; the outer `uris` array
    ...
    // `ENC_REQUIRES_CONTENT_HASH` pre-check (Label 309 §4.4); the field is captured
  ...
  .superRefine((item, ctx) => {
    // Label 309 §4.4 content-hash pre-check: when `enc` is present, `item.hashes`
    ...
    if (
      ...
      if (!hasContentHash) {
        ctx.addIssue({
          ...
          path: ['enc'],
          message:
            'item carries `enc` but `hashes` has no content-hash entry (sha2-256 or blake2b-256) per Label 309 §4.4',
    ...
    // Per Label 309 §4.2 and Label 309: `uris` is
    ...
    // (Label 309). The structural validator MUST NOT reject on that ground;
      ...
      item.uris.forEach((chunks, ui) => {
        ...
        if (reconstructed.includes('#')) {
          ctx.addIssue({
            code: 'custom',
            path: ['uris', ui],
            message: 'URI contains fragment identifier (`#`) — forbidden per Label 309 §4.2',
        ...
        if (!/^[a-z][a-z0-9+.-]*:\/\//i.test(reconstructed)) {
          ...
          // Per-scheme shape rules (Label 309 §4.8.1).
          if (reconstructed.startsWith('ar://')) {
            ...
            // Per Label 309 §4.8.1, conformant validators MUST do full CID parsing
...
// Top-level `merkle[]` list-commitment entry (Label 309 §4.5). Shape:
...
// `leaf_count` is REQUIRED (Label 309 §4.5) and commits the leaf count alongside
// the root; mismatch against the off-chain leaves-list value surfaces at the
// verifier layer as `SCHEMA_MERKLE_LEAF_COUNT_MISMATCH` (Label 309).
...
// Label 309 §4.11. JavaScript `===` does not distinguish 1 from 1.0, and a
...
// least one of `items[]` or `merkle[]` non-empty (Label 309 §4.2 / Label 309 §4.5);
// the structural validator rejects an empty record with `SCHEMA_EMPTY_RECORD`.
//
// Forward-compat (Label 309 §4.1.4): extension keys matching `^x-.+` or
...
// `EXTENSION_UNSUPPORTED_CRITICAL` (Label 309) and MUST NOT report valid.
export const PoeRecordSchema = z
  .looseObject({
    v: VersionLiteralSchema,
    items: z.array(ItemEntrySchema).optional(),
    // Top-level Merkle list commitments (Label 309 §4.5). OPT. Each entry pairs a
    ...
    // Label 309 §4.6.3. No parallel-array invariant: the optional `cose_key` lives
    // inside the entry.
    sigs: z.array(SigEntrySchema).min(1).optional(),
    // Forward-compat critical-extension array (Label 309 §4.1.4).
...
function mapZodIssue(i: z.core.$ZodIssue): ValidationIssue {
  ...
  // Per Label 309 §4.6.3 / Label 309: sig-entry structural violations have a
  ...
  switch (i.code) {
    case 'invalid_type':
      ...
      // Per Label 309, missing top-level required fields emit
...
export function validatePoeRecord(bytes: Uint8Array): ValidationResult {
  ...
  try {
    ...
    // Per Label 309, duplicate-key violations are a subclass of `MALFORMED_CBOR`.
  ...
  // Note: Label 309 v1 requires `hashes` to carry at least one entry from the
  // §4.10.2 registry; single-hash records are fully conformant and structural
  // validators emit no `SINGLE_HASH`-style warning (Label 309 §4.3, Rationale).

  // `crit[]` shape checks (Label 309 §4.1.4 / Label 309, code
  ...
  // Forward-compat `crit` enforcement (Label 309 §4.1.4 / Label 309). A v1
  ...
  // per Label 309 §4.6.3. We decode COSE_Sign1 from the joined `cose_sign1` chunks here.
  //
  // The optional `cose_key` (chunked cbor<COSE_Key>, path 2) MUST also be
  // structurally inspected here: per Label 309 §4.6.3, the validator MUST decode
    ...
    record.sigs.forEach((entry, i) => {
      ...
      try {
        ...
        if (typeof alg !== 'number' || !KNOWN_SIG_ALG_IDS.has(alg)) {
          // Label 309: info severity. The content claim under `hashes`
        ...
        // Path 1 / path 2 mutual exclusion at the wire level (Label 309 §4.6.3).
        ...
        if (
          ...
          issues.push({
            ...
            message:
              'sigs[i] carries both a 32-byte protected `kid` (path 1) and an inline `cose_key` (path 2); paths are mutually exclusive per Label 309 §4.6.3',
          });
        }
      } catch (e) {
        issues.push({
...
// (Label 309 §4.6.3). Concatenates the chunks and decodes the bytes as a CBOR map,
  ...
  if (hasLabel(-4)) {
    return {
      ...
      message:
        'sigs[' +
        i +
        '].cose_key carries COSE_Key label -4 (private scalar `d`); publishing private-key material on chain is forbidden per Label 309 §4.6.3',
    };
  }
