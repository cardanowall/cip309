// Label 309 v1 reference implementation — Label 309 record encoder
// Spec: Label 309
...
// content-hash identifier from Label 309 §4.10.2 round-trips through canonical
// CBOR verbatim.
//
// Top-level `merkle[]` (Label 309 §4.5, OPT) is peer to `items`/`sigs` and is
...
// `merkle[i].leaf_count` (REQUIRED, Label 309 §4.5) commits the leaf count
...
// `cose_sign1` (per Label 309 §4.6.3); when the optional `cose_key` companion
...
// the literal field name `passphrase` (per Label 309 §4.4); the algorithm value
// `argon2id` (final spelling, Label 309 §4.10.6) replaces any legacy `-v13`
// suffix everywhere.

import { encodeCanonicalCbor } from './cbor-canonical.ts';
import type { PoeRecord } from './label-309-validator.ts';

// Label 309 v1 record-level signature domain separator (25 bytes UTF-8).
// Per Label 309 §4.6.1, this prefix is embedded at the start of Sig_structure[3]
...
// Per Label 309 §4.6.1, the signed bytes are:
...
// `sigs` is the only field that gets stripped (per Label 309 §4.6.3, the optional
...
// Label 309 carries signatures at the record level only; there is no per-item
// signature slot and no per-item payload-encoder. Authorship for multi-author
// content uses one PoE per author. See Label 309 §4.6 and the Rationale section
